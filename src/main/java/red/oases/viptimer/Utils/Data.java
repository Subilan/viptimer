...
public class Data {
    ...
    public static boolean increaseRecvCount(String distId) {
        return increment("receipt", "recv_count", "dist_by='%s' AND recv_by='%s'".formatted(distId, Sync.getInstanceId()));
    }
    ...
    public static List<Distribution> getDistributions() {
        ...
        var distNotReceived = withResult(
                "SELECT * FROM distribution WHERE NOT dist_by IN (SELECT dist_by FROM receipt WHERE recv_by='%s')".formatted(Sync.getInstanceId()),
                collectDistribution
        );
    ...
    public static boolean createDelivery(String playername, String type) {
        return execute("INSERT INTO delivery (playername, type, inst_id) VALUES ('%s', '%s', '%s')"
                .formatted(playername, type, Sync.getInstanceId()));
    }
    ...
    public static boolean hasReceipt(String distId) {
        return hasResult("SELECT * FROM receipt WHERE dist_by='%s' AND recv_by='%s'".formatted(distId, Sync.getInstanceId()));
    }

    public static boolean createReceipt(String distId) {
        return execute("INSERT INTO receipt (dist_by, recv_by) VALUES ('%s', '%s')".formatted(distId, Sync.getInstanceId()));
    }
